...
class CASCADEEnv(gym.Env):
    ...
    def step(self, action: np.ndarray) -> Tuple[Obs, float, bool, bool, Info]:
        ...
        # 8. 概率性地注入随机事件
        # 9. 从有就绪任务的子区域中随机选择下一步聚焦区域（固定 seed 可复现）
...
**Episode 结构**：一个 episode 覆盖一次完整灾情场景，包含该场景下全部子区域（2~5 个）的所有任务链从头到尾执行完毕。**每步聚焦一个子区域**，环境从当前有就绪任务的区域集合中随机选择下一步聚焦区域，随机过程由 `env.seed` / `reset(seed=...)` 控制，可复现——当前步只看到当前区域的待调度任务，但所有 UAV 是跨区域共享的。
...
Episode #k:
  ...
  → 区域选择序列: A → C → A → B → C → B → ...（固定 seed 下可复现，非循环）
```
...
**区域选择策略说明**：
- 默认使用 `region_selection: random_ready`，每个调度步结束后从仍有就绪任务的区域集合中随机选择下一步聚焦区域
- 随机序列由 `env.seed` 或 `reset(seed=...)` 固定，同一 seed 下训练/评估轨迹可复现
- 当某区域的就绪任务为空时（所有任务要么完成要么被阻塞），自动从候选集合中排除该区域
- 可选兼容模式：`region_selection: first_ready` 保留按 ready 列表第一个区域选择的确定性策略；后续也可由调度器学习区域选择策略（如优先调度有高 priority 任务的区域）
...
**整体完成状态（2026-05-27）**：算法框架设计已完成并通过远端 GPU 服务器验证。验证环境为远端 `.venv` 中 `torch=2.6.0+cu124`、CUDA `12.4`、GPU `NVIDIA GeForce RTX 4080`；已运行 `python -m unittest tests.test_algorithms.test_cascade`、`python -m unittest discover -s tests` 和 `python experiments/run_cascade.py --episodes 1 --seed 0 --swanlab-mode disabled --no-progress --output-dir outputs/results/remote_algorithm_framework_recheck`，全部通过。完整异步多环境训练与长 episode 收敛实验属于 Phase 3 训练工程，不属于本节“算法框架设计”的完成条件。

### 3.1 调度算法抽象基类

**实现状态（2026-05-27）**：完成，已远端测试。`src/algorithms/base_scheduler.py` 已提供统一 `observe()`、`decide()`、`learn()`、`reset()`、`save()`、`load()` 接口；启发式方法复用空学习接口，CASCADE 学习型调度器实现模型保存/加载。
...
**实现状态（2026-05-27）**：完成，已远端测试。多 Agent Actor-Critic、DAG/网络拓扑/资源/多跳特征融合、并发任务-UAV 动作矩阵、Hungarian 离散化、6 维资源和距离感知网络均已进入代码框架。
...
**实现状态（2026-05-27）**：框架完成，已远端测试。`ma3c_trainer.py` 已实现 `MA3CConfig`、`compute_gae()`、`learn()` 的 rollout 统计入口和模型 checkpoint；完整异步多环境采样、反向传播更新和 target network 软更新属于 Phase 3 训练工程，会在当前接口上继续接入。
...
**实现状态（2026-05-27）**：完成，已远端测试。环境侧 `src/env/action_mask.py` 负责生成二维可行动作掩码；算法侧 `src/algorithms/cascade/hungarian_match.py` 使用 SciPy Hungarian 在掩码内做一对一匹配，并忽略分数为 0 的候选组合。
...
**当前实现口径（2026-05-27）**：先采用简单、经典、可稳定复现的 4 类 baseline 与 CASCADE 对比：Greedy、Min-Load、Round-Robin、HEFT。重型 DRL/优化方法（DECCo、MADDPG、QMIX、PPO/DQN、MILP、GA 等）保留为后续扩展，不作为当前训练与主流程打通的阻塞条件。
...
**当前状态**：暂缓。DECCo 复现需要单独对齐论文的 mA2C 状态、动作和 3 项成本函数，属于后续复杂对比方法；当前先用经典启发式 baseline 完成可复现实验闭环。
...
**实现状态（2026-05-27）**：完成。DS1/DS2/DS3 已分别落地为 `configs/env/scenario_ds1_standard.yaml`、`configs/env/scenario_ds2_complex.yaml`、`configs/env/scenario_ds3_extreme.yaml`；DS1/DS2 参数范围已按本节表格对齐，DS3 保留 UAV 故障和应急任务注入。
...
**当前实现口径（2026-05-27）**：先打通轻量实验矩阵 E1-Simple/E2-Simple/E3-Simple，统一使用 CASCADE、Greedy、Min-Load、Round-Robin、HEFT。运行入口为 `experiments/run_simple_comparison.py`，可一次覆盖 DS1/DS2/DS3，也可通过 `--scenarios`/`--methods` 选择子集。

| 实验编号 | 名称 | 场景 | 对比方法 | 关键指标 | 核心验证目标 |
|---------|------|------|---------|---------|-------------|
| **E1-Simple** | 主对比实验 | DS1 | CASCADE, Greedy, Min-Load, Round-Robin, HEFT | Makespan, ATCT, $\bar{U}_{gpu}$, $\sigma_{gpu}$ | 验证 CASCADE 与经典方法的基础差异 |
| **E2-Simple** | 跨场景泛化 | DS2 | CASCADE, Greedy, Min-Load, Round-Robin, HEFT | 全部 6 项，关注 DS1→DS2 退化率 | 验证复杂场景下的泛化趋势 |
| **E3-Simple** | 极端/降级鲁棒性 | DS3 | CASCADE, Greedy, Min-Load, Round-Robin, HEFT | Makespan 增幅, $\sigma_{gpu}$, PTCT | 验证故障/应急注入下的鲁棒性 |
...
**实现状态（2026-05-27）**：训练与评估闭环已打通。`experiments/train_cascade.py` 支持 DS1 训练、从 `--checkpoint` 恢复、周期性验证、GAE/Actor-Critic 更新、checkpoint 和训练指标输出；`experiments/run_simple_comparison.py` 支持通过 `--checkpoint` 加载训练后的 CASCADE，与经典 baseline 在 DS1/DS2/DS3 上统一评估。CASCADE 默认按 `--model-num-uavs 15` 建模，保证 DS1 训练 checkpoint 可评估 DS2/DS3。当前训练实现为单进程端到端流程，异步 `n_envs=8` A3C 属于后续加速增强。
...
16.                    # step() 内部按固定 seed 随机选择下一个有就绪任务的子区域
