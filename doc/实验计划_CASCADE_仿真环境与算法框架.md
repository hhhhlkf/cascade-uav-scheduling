...
**实现状态（2026-05-27）**：训练与评估闭环已打通。`experiments/train_cascade.py` 支持 DS1 训练、周期性验证、GAE/Actor-Critic 更新、checkpoint 和训练指标输出；`experiments/run_simple_comparison.py` 支持训练后或当前策略与经典 baseline 在 DS1/DS2/DS3 上统一评估。当前训练实现为单进程端到端流程，异步 `n_envs=8` A3C 属于后续加速增强。
